...
// for any other invocation prints the CTXMODE_ENV_PROBE marker (a positive
// probe that proves the shim actually ran and inherited the non-sensitive
// marker) plus the FAKE_* env vars the subprocess actually inherited — a
// direct probe of the child environment.
func writeFakeGit(t *testing.T, wd string) string {
	t.Helper()
	fakeDir := t.TempDir()
	script := "#!/bin/sh\ncase \" $* \" in\n  *\" rev-parse \"*) echo " + wd + " ;;\n"
	script += "  *) env | grep '^CTXMODE_ENV_PROBE=' || true; env | grep '^FAKE_' || true ;;\n"
	script += "esac\n"
...
func TestRunGit_StripsSensitiveEnv(t *testing.T) {
	...
	if !strings.Contains(out, "CTXMODE_ENV_PROBE=1") {
		t.Fatalf("fake git shim must have run and inherited the marker (probe absent): %q", out)
	}
